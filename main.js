...
let known = JSON.parse(readFileSync('ids.json', 'utf-8'))


async function fetch_steam(item) {
    try {
        let id = (await get(`http://${server_ip}:${server_port}/getid/${item.market_hash_name}`)).data
        if (id == -1) {
            console.log(`Unable to fetch ${item.market_hash_name}`)
            return Promise.resolve()
        }
        let steam_highest_buy = (await get(`https://steamcommunity.com/market/itemordershistogram?language=english&currency=${currency}&item_nameid=${id}`)).data.buy_order_graph[0][0]
...
async function get_prices(min, max) {
    let price_list = await get('https://market.csgo.com/api/v2/prices/RUB.json')
...
async function get_missing_ids() {
    let price_list = (await get('https://market.csgo.com/api/v2/prices/RUB.json')).data.items
